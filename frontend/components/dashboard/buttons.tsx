...
type ThemeKey = "dotcom" | "financial" | "covid";
...
function DotComIcon({ color = "currentColor", size = 18 }) {
  return (
...
function FinancialIcon({ color = "currentColor", size = 18 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
...
function CovidIcon({ color = "currentColor", size = 18 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
...
// ─── Radio Dot — always visible, signals single-select ───────────────────────
function RadioDot({ selected, accent, accentRgb }: { selected: boolean; accent: string; accentRgb: string }) {
  return (
    <div style={{
      width: 22,
      height: 22,
...
// ─── Card ─────────────────────────────────────────────────────────────────────
function CrisisCard({ variant, selected, dimmed, onClick }: { variant: ThemeKey; selected: boolean; dimmed: boolean; onClick: () => void }) {
...
// ─── Detail Panel ─────────────────────────────────────────────────────────────
function DetailPanel({ variant }: { variant: ThemeKey }) {
  const theme = themes[variant];
  return (
    <motion.div
      key={variant}
...
// ─── Main (controlled when onSelectPeriod is provided) ─────────────────────────
export function CrisisButtonGroup({
  ...
  className?: string;
  /** Controlled: selected crisis period (dotcom | financial | covid). */
  selectedPeriod?: ThemeKey | null;
  /** When set, period is controlled from parent (e.g. for attaching dates to stock request). */
